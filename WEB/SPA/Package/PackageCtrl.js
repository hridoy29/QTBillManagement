app.controller("PackageCtrl", function ($scope, $http, blockUI) {
    ...
    function clear() {
        $scope.entity = { PackageId: 0, IsActive: true };
        $("#txtFocus").focus();
    };
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/Package/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "PackageName = '" + $scope.entity.PackageName + "'";
        if ($scope.entity.PackageId > 0)
            where += " AND PackageId <> " + $scope.entity.PackageId;

        $http({
            url: '/Package/GetDynamic?where=' + where + '&orderBy=PackageName',
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
                alertify.log($scope.entity.PackageName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
                    trnType = $scope.entity.PackageId === 0 ? "INSERT" : "UPDATE";
