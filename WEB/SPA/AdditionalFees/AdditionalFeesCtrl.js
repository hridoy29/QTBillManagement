app.controller("AdditionalFeesCtrl", function ($scope, $http, blockUI) {
    ...
    function clear() {
        $scope.entity = { AdditionalFeeId: 0, IsActive: true };
        $("#txtFocus").focus();
    };
    ...
    function submitRequest(trnType) {
        ...
        $http.post('/AdditionalFee/Post', params).success(function (data) {
    ...
    $scope.post = function (trnType) {
        var where = "AdditionalFeeName = '" + $scope.entity.AdditionalFeeName + "'";
        if ($scope.entity.AdditionalFeeId > 0)
            where += " AND AdditionalFeeId <> " + $scope.entity.AdditionalFeeId;
        ...
        $http({
            ...
            if (data.length > 0) {
                alertify.log($scope.entity.AdditionalFeeName + ' already exists!', 'already', '5000');
                $('#txtFocus').focus();
            } else {
                if (trnType === 'save') {
                    trnType = $scope.entity.AdditionalFeeId === 0 ? "INSERT" : "UPDATE";
